...
parameters.Tf(1) =      3.0000;  % minimum temperature (C) for growth                
parameters.Tf(2) =     16.0000;  % growth rate is max in the range T2-T3 (lower optimal temperature, C)   
...
parameters.Wf(1) =     0.0100;  % minimum soil moisture for growth (v/v)
parameters.Wf(2) =     0.0150;  % the growth rate is max in range W2-W3 (lower optimal soil moisture, v/v)
...
parameters.SNo  =    300.0000;  % initial snowpack (mm)

parameters.Wo    =      0.2183;  % initial soil moisture (v/v)
parameters.Wmax  =      0.7000;  % maximum soil moisture (field capacity, (v/v)
parameters.Wmin  =      0.0100;  % minimum soil moisture (wilting point, v/v)
parameters.rootd =     500.000;  % the root/soil melt depth (mm)

parameters.rated =      0.0051;  % the rate of water drainage from soil
parameters.Pmax  =     30.0000;  % maximum rate of infiltration water into soil  (mm/day)

% interception and transpiration parameters
parameters.k(1) =      0.9000;  % k1 (1-k1) is the interception precipitation by the tree crown
parameters.k(2) =      0.2500;  % 1st coefficient for calculation the transpiration
parameters.k(3) =      0.1800;  % 2nd coefficient for calculation the transpiration

% soil and snow melting parameters
parameters.Tm   =      20.000;  % sum of temperature for start soil melting (C)
parameters.a(1) =     10.0000;  % 1st coefficient of soil melting
parameters.a(2) =      0.0060;  % 2nd coefficient of soil melting
parameters.Tg   =     30.0000;  % sum of temperature to start growth (C)
parameters.SNr  =      3.0000;  % the rate of snow melting (mm/C/day)
...
parameters.K(9)  =         14;  % the period (days) over which to sum temperature to calculate start of growth
...
parameters.b(7)  =        .42;  % b(7) and b(6) determine the fuction Vo(j)
