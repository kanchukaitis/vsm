...
parameters.Wf(1) =      .0400;  % minimum soil moisture for growth (v/v)
...
parameters.Wo   =       .0900;  % initial soil moisture (v/v)
parameters.Wmax =       .3500;  % maximum soil moisture (field capacity, (v/v)
parameters.Wmin =       .0400;  % minimum soil moisture (wilting point, v/v)
parameters.rootd=    1000.000;  % the root/soil melt depth (mm)

parameters.rated=       .0010;  % the rate of water drainage from soil
parameters.Pmax =     20.0000;  % maximum rate of infiltration water into soil  (mm/day)

% interception and transpiration parameters
parameters.k(1) =       .7200;  % k1 (1-k1) is the interception precipitation by the tree crown
parameters.k(2) =       .1200;  % k2 coefficient for calculation the transpiration
parameters.k(3) =       .1750;  % k3 coefficient for calculation the transpiration

% soil and snow melting parameters
parameters.Tm   =       0.000;  % sum of temperature for start soil melting (C)
parameters.a(1) =     20.0000;  % a1, coefficient of soil melting
...
parameters.b(7)  =        .25;  % b(7) and b(6) determinate the fuction  Vo(j)
